  ...
  if (!vm.$options.render) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if ((vm.$options.template && vm.$options.template.charAt(0) !== '#') ||
        vm.$options.el || el) {
        warn(
          'You are using the runtime-only builder of Vue where the template ' +
          'compiler is not available. Either pre-compile the templates into ' +
          'render functions, or use the compiler-included builder.',
          vm
        );
      } else {
        warn(
...
function flushSchedulerQueue () {
  ...
  for (index = 0; index < queue.length; index++) {
    ...
    has[id] = null;
    watcher.run();
    // in dev builder, check and stop circular updates.
...
function stateMixin (Vue) {
  ...
  // when using Object.defineProperty, so we have to procedurally builder up
  // the object here.
  var dataDef = {};
