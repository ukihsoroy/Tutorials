var {{widgetname}} = StudioWidgetWrapper.extend({
    ...
    render : function()
    {
        ...
        if(elem)
        {
			...
			var vm = new Vue({
				el: $("#{{widgetname}}", elem)[0],
				template: '<{{name}}_component ref="component" v-bind="widgetProperties"></{{name}}_component>',
				i18n: i18n,
				widget: thisObj,
                studio: Studio,
                data:{
                        ...
                        Object.keys(props).forEach(propName => {
                            ...
                            if(propsDataValue){
                                if(!type || type === String){
                                    ...
                                    propsData[propName] = (propsDataValue === 'true' || propsDataValue === true) ? true: false;
                                }else if(type === Number){
